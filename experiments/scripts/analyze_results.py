...
def compute_max_batch_size(
    ...
    filtered = [
        ...
        if row.get("mode") == mode and abs(row.get("wait_seconds", 0) - wait_seconds) < 0.01
    ]
...
def generate_protocol_comparison_table(
    ...
    for infra_id, data in sorted(results.items()):
        for wait in wait_times:
            ...
            lines.append(
                f"| {infra_id:<14} | {wait:<6.1f} | {ws_str:^6} | {http_str:^8} | "
                f"{ratio_str:^13} | {winner:^6} |"
            )
    ...
    for mode in ["ws", "http"]:
        for wait in [0.1, 1.0, 5.0]:
            ...
            if row:
                ...
                lines.append(
                    f"| {mode:<4} | {wait:<6.1f} | {max_batch or '-':^9} | "
                    f"{p99:.2f}s | {success:.1f}% | {rps:.1f} |"
                )
            else:
...
def main():
    ...
    if len(results) == 1:
        ...
        print(generate_results_summary(
            results[infra_id], infra_id, args.success_threshold
        ))
